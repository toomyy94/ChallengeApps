...
//            GeoLocationManager.getInstance(LastfmApplication.this).sleepTracking();
        }
    ...
    private class CoreActivityLifecycleCallbacks implements Application.ActivityLifecycleCallbacks {
...
//            LOGGER.debug("Activity resumed: {}", activity.getClass().getName());
            if (isInBackground) {
//                GeoLocationManager.getInstance(LastfmApplication.this).wakeTracking();
            }
            synchronized (locker) {
